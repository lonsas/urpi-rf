...
class RFDevice:
    ...
    def __init__(
        ...
        self.tx_gpio = Pin(tx_gpio, Pin.IN, None)
        ...
        # internal values
        if rx_gpio is None:
            # Make it compatible with rpi_rf
            rx_gpio = tx_gpio
        self.rx_gpio = Pin(rx_gpio, Pin.IN)
        ...
        _LOGGER.debug("Using GPIO Tx " + str(tx_gpio) + " Rx " + str(rx_gpio))
    ...
    def enable_tx(self):
        ...
        if not self.tx_enabled:
            ...
            self.tx_gpio.init(Pin.OUT, value=0)
    ...
    def tx_code(self, code, tx_proto=None, tx_pulselength=None, tx_length=None):
        ...
        if tx_length:
            ...
            self.tx_length = 24
        bincode = bin(code)[2:]
        rawcode = "0" * (self.tx_length - len(bincode)) + bincode
        if self.tx_proto == 6:
            nexacode = ""
            for b in rawcode:
                if b == "0":
    ...
    def enable_rx(self):
        ...
        if not self.rx_enabled:
            ...
            self.rx_gpio.init(Pin.IN)
            self.rx_gpio.irq(self._rx_callback, trigger=Pin.IRQ_FALLING | Pin.IRQ_RISING)
    ...
    def disable_rx(self):
        ...
        if self.rx_enabled:
            self.rx_gpio.irq(None, trigger=0)
            self.rx_enabled = False
            _LOGGER.debug("RX disabled")
        return True

    # pylint: disable=unused-argument
    def _rx_callback(self, gpio):
                ...
                if self._rx_repeat_count == 2:
                    for pnum in range(1, len(PROTOCOLS)):
                        if self._rx_waveform(pnum, self._rx_change_count, timestamp):
                            _LOGGER.debug("Protocol: " + str(pnum) + " RX code: " + str(self.rx_code))
                            if self.rx_callback is not None:
                                self.rx_callback(self.rx_proto, self.rx_code)
                            break
                    self._rx_repeat_count = 0
            self._rx_change_count = 0
